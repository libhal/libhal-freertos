...
using sample_t = hal::stream_dac<std::uint8_t>::samples;
  ...
  while (true) {
    ...
    if (pcm8_span.empty()) {
      sample_t finished_samples = {
        ...
        .samples = { pcm8_array[buffer_index] },
      };
...
void application(hardware_map_t& p_map)
{
  ...
  static std::array<StackType_t, 128> audio_stream_stack{};
  [[maybe_unused]] constexpr size_t audio_stream_stack_size =
    sizeof(audio_stream_stack);
  static StaticTask_t audio_stream_tcb;
  xTaskCreateStatic(&audio_streamer,
                    "stream",
                    audio_stream_stack.size(),
                    nullptr,
                    4,
                    audio_stream_stack.data(),
                    &audio_stream_tcb);

  static std::array<StackType_t, 6000> audio_decoder_stack{};
  [[maybe_unused]] constexpr size_t audio_decoder_stack_size =
    sizeof(audio_decoder_stack);
  ...
  static StaticTask_t audio_decoder_tcb;
  xTaskCreateStatic(&audio_decoder,
                    "decoder",
                    audio_decoder_stack.size(),
                    nullptr,
                    4,
                    audio_decoder_stack.data(),
                    &audio_decoder_tcb);
