...
extern "C"
{
   ...
   *
   * This structure differs from the gcc/llvm objects in that the 3 field is
   * not a union of an unsigned value and a pointer and in this case is just a
   * pointer.
   *
   */
  struct __emutls_object
  {
     ...
     *
     * When this value is initially accessed, the value will be set to the
     * size of the TCB object.
     *
    ...
    /**
     * @brief pointer to the position of the tls object in the TLS memory
     * region
     *
     ...
     * initialization process. This value is a relative offset to the where
     * the thread local storage region is. The new location must not be zero
     * as it would be reinitialized on each access of the object. THUS, the
     * thread allocation libraries must over allocate the TCB area.
     */
    void* offset_pointer;
    /**
